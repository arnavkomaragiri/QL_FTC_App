...
@Autonomous(name = "QL_Auto_R1_SAMPLE_ONLY", group = "Competition")
    ...
    public void loop(){
        switch (mRobotState){
            ...
            case STATE_ALIGN:
                ...
                if (drive.goTo(new Pose2d(-24 * Math.sqrt(2), 12 * Math.sqrt(2), 315), telemetry, 1.0, 7)){// && arm.getmTransferState() == 1){
                    memo = drive.getPos();
                    //drive.disengage();
                    drive.engage();
            ...
            case STATE_CENTER:
                ...
                if (drive.goTo(new Pose2d(x, 1, 0), telemetry, 0.4, 1)){
                    newState(State.STATE_SAMPLE);
                }
                break;
            case STATE_TRANSFER:
