 ...
 */
@Autonomous(name = "State: QL_Auto_R2_X",group = "State:")
@Disabled
public class QL_Auto_R2_X extends OpMode{
    ...
    private ElapsedTime mGameTime = new ElapsedTime();
        ...
        switch (mRobotState){
            ...
            case STATE_LEAVE_PLATFORM:
                ...
                if (leave_platform()) {
                    ...
                    //sigma = gyro.getIntegratedZValue();
            ...
            case STATE_PREPARE:
                if (mStateTime.time() >= 0.5) {
                    if (gyroTurn4(velocity, 270, cone)) {
            ...
            case STATE_POSITION:
                if (mStateTime.time() >= 0.25) {
                    //standard:
                    if (gyroTurn6(velocity, 330)) {
                        logMessage("Entering Travel:", Integer.toString(gyro.getIntegratedZValue()));
                        newState(State.STATE_TRAVEL);
                        //break standard;
                    }
                    if (mStateTime.time() >= 2.5) {
                        if (!(gyro.getIntegratedZValue() >= 225 && gyro.getIntegratedZValue() <= 310)) {
                            logMessage("Kickout:", Integer.toString(gyro.getIntegratedZValue())); //todo: add safety for kill zone
                            newState(State.STATE_TRAVEL);
                        } else {
            ...
            case STATE_SWIVEL:
                if (mStateTime.time() >= 0.25) {
                    ...
                    if (mStateTime.time() >= 3.5) {
                        if (!(gyro.getIntegratedZValue() >= 180 && gyro.getIntegratedZValue() <= 330)) {
                            logMessage("Kickout:", Integer.toString(gyro.getIntegratedZValue())); //todo: add safety for kill zone
                            newState(State.STATE_BACK);
                        } else {
                            logMessage("Abort:", Integer.toString(gyro.getIntegratedZValue()));
    ...
    private void newState (QL_Auto_R2_X.State newState){
        // Reset State time and change to next state
        logMessage("ODR:", Double.toString(odr.getLightDetected()));
        mStateTime.reset();
        mRobotState = newState;
        mResetEncoder = false;
    }
    private void newturnState (QL_Auto_R2_X.TURN_STATE newTurnState){
        mTurnStateTime.reset();
        turnstate = newTurnState;
        mResetEncoder = false;
    }
    private void newgrabState (QL_Auto_R2_X.GRAB_STATE newGrabState){
        mGrabTime.reset();
    ...
    private boolean leave_platform () {
        ...
        int forwardcounts = myUtils.getNewCounts(21,myUtils.NEVEREST60,3);
    ...
    private boolean rendezvous(int distance) {
        ...
        if (up_left.getCurrentPosition() <= (5 * (forwardcounts / 6))){
            ...
            back_left.setPower(-0.8);
            back_right.setPower(-0.8);
            logMessage("Going Forward:", Integer.toString(up_left.getCurrentPosition()));
        }
        else {
        ...
        telemetry.addData("C Position",up_left.getCurrentPosition());
        telemetry.addData("T Position",up_left.getTargetPosition());
        if (Math.abs(up_left.getCurrentPosition())>=forwardcounts ||
                Math.abs(up_right.getCurrentPosition()) >=forwardcounts ||
                Math.abs(back_left.getCurrentPosition()) >= forwardcounts ||
                Math.abs(back_right.getCurrentPosition()) >= forwardcounts){
            up_left.setPower(0.0);
            up_right.setPower(0.0);
            back_right.setPower(0.0);
            back_left.setPower(0.0);
    ...
    boolean gyroTurn4 (  double speed, int angle, int cone) {
        ...
        if (deviation >= -1 * cone && deviation < 0){
            //if ((mStateTime.time() % 0.5) == 0) {
            velocity = velocity / 2;//a bit manual, to be fixed later
            //}
            if (velocity < 0.2)
    ...
    boolean gyroTurn5 (  double speed, int angle) {
        ...
        if (deviation <= 60 && deviation > 0 && !reached){
            //if ((mStateTime.time() % 0.5) == 0) { //mod used to slow down motors every second
            velocity = velocity / 2;
            //}
            if (velocity < 0.2)
        ...
        if (deviation >= -60 && deviation < 0 && !reached){
            ...
            logMessage("Negative Slowing Down:", Integer.toString(deviation));
            if (Math.abs(deviation) < 0) { //Math.abs(gyro.getIntegratedZValue) < angle
                up_left.setPower(0.0);
                up_right.setPower(0.0);
                back_left.setPower(0.0);
                back_right.setPower(0.0);
        ...
        if (initial){
            switch (turnstate){
                ...
                case STATE_2:
                    ...
                    if (mTurnStateTime.time() >= 0.25) {
                        if (gyroTurn4(velocity, 180, 60)) {
                            up_left.setPower(0.0);
                            back_left.setPower(0.0);
                            up_right.setPower(0.0);
                            back_right.setPower(0.0);
