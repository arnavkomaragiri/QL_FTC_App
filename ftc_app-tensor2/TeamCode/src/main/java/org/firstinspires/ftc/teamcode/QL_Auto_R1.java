    ...
    public void loop(){
        switch (mRobotState){
            case STATE_START:
                ...
                //pos = 0;
                //arm.move(-1.0, 2, true);
                //if (Math.abs(arm.getArm().getCurrentPosition() + 1400) < 10){
            ...
            case STATE_DELATCH:
                ...
                break;
            case STATE_DELATCH2:
                if (drive.goTo(new Pose2d(3, 1, 0), telemetry,0.5, 1)){
                    telemetry.addData("Moving to sample: ", pose.toString());
            ...
            case STATE_SAMPLE:
                ...
                switch (pos){
                    case 0:
                        dest = new Pose2d(-14 * Math.sqrt(2), 12 * Math.sqrt(2), 0);
                        break;
                    case 1:
                        dest = new Pose2d(0, 12 * Math.sqrt(2), 0);
                        break;
                    case 2:
                        dest = new Pose2d(14 * Math.sqrt(2), 12 * Math.sqrt(2), 0);
                        break;
                ...
                if (drive.goTo(dest, telemetry)){// && !dest.equals(new Pose2d(0, 0, 0))){
                    //kill motors
                    //arm.move(0.0, 2, true);
                    switch (pos) {
                        case 0:
            ...
            case STATE_TRAVEL:
                ...
                if (drive.goTo(new Pose2d((-48 * Math.sqrt(2)), 12 * Math.sqrt(2), 315), telemetry, 0.5, 25, 1)){
                    drive.motor_reset();
            ...
            case STATE_TRAVEL2:
                ...
                break;
            case STATE_TURN3:
                telemetry.addData("Turning Again Again: ", drive.getRobotHeading());
                if (mStateTime.time() >= 0.25) {
                    if (drive.turn(-43, 2, 0.25, telemetry)) {
                        //drive.setPos(new Pose2d(12 * Math.sqrt(2), -24 * Math.sqrt(2), 315));
                        //drive.engage();
                        newState(State.STATE_RETURN);
                    }
            ...
            case STATE_RETURN:
                ...
                /*if (drive.goTo(new Pose2d(0 * Math.sqrt(2), 48 * Math.sqrt(2), 315), telemetry, 0.5, 5, 1)){
                    ...
                    if (drive.getOdoDistance() * Math.sin(Math.toRadians((this.drive.getRobotHeading() - 315) + 90)) >= 32) {
                        drive.drive(0.0, 0.0, 0.0, 0.0);
                        drive.motor_reset();
                        newState(State.STATE_STOP);
                    }
